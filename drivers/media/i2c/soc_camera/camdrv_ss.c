...
static DEVICE_ATTR(rear_camtype, 0444, maincamtype_show, NULL);
static DEVICE_ATTR(rear_camfw, 0444, maincamfw_show, NULL);
static DEVICE_ATTR(front_camtype, 0444, subcamtype_show, NULL);
static DEVICE_ATTR(front_camfw, 0444, subcamfw_show, NULL);
...
static DEVICE_ATTR(rear_vendorid, 0444, camera_rear_vendorid_show, NULL);
...
static int camdrv_ss_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *v4l2Ctrl);
	...
	switch (v4l2Ctrl->id) {
		case V4L2_CID_CAMERA_READ_MODE_CHANGE_REG:
		{
			if (atomic_read(&gCapModeState) ==
				...
				CAM_INFO_PRINTK("%s %s : CAPTURE MODE STILL PROCESSING....\n",
									sensor.name, __func__);
		...
		case V4L2_CID_CAMERA_READ_CAM_MODE:
			v4l2Ctrl->value = (state->current_mode == PICTURE_MODE ? 1 : 0);
			break;
