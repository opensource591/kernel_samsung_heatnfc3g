...
static int bsc_wait_cmdbusy(struct bsc_i2c_dev *dev)
{
	...
	limit = (loops_per_jiffy * msecs_to_jiffies(dev->ses_timeout));
					      ...
					      unsigned int len)
{
	...
	/* drain RX FIFO before any data transactions */
	if (!bsc_rx_fifo_is_empty((uint32_t)dev->virt_base)) {
		dev_err(dev->device, "RX FIFO not empty. Draining FIFO...\n");
		while (!bsc_rx_fifo_is_empty((uint32_t)dev->virt_base)) {
			uint8_t garbage;
			garbage = bsc_read_from_rx_fifo((uint32_t)dev->virt_base);
			dev_err(dev->device, "RX garbage read %2.2X\n", garbage);
		}
	}
	...
	bsc_disable_intr((uint32_t)dev->virt_base,
			 I2C_MM_HS_IER_READ_COMPLETE_INT_MASK);
	if (time_left == 0) {
		dev_err(dev->device, "RX FIFO time out, draining RX FIFO...\n");
		/* draining RX FIFO */
		while (!bsc_rx_fifo_is_empty((uint32_t)dev->virt_base)) {
			uint8_t garbage;
			garbage = bsc_read_from_rx_fifo((uint32_t)dev->virt_base);
				...
				unsigned int retries)
{
	...
	for (i = 0; i <= retries; i++) {
		...
		// NFC wake up takes 370usec
		// nfc uses write NAK as a wakeup source
	}
...
static int bsc_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.platform_data) {
		...
		if (of_property_read_u32(pdev->dev.of_node, "addr_retries",
			&val))
			dev->addr_retries = 1;
		else
			dev->addr_retries = val;
