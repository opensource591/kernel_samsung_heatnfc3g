...
void trace_mark_begin(
		...
		const char* args) {
	if (trace_mark_enabled()) {
		trace_mark('B', current->pid, title, args, current->comm);
	}
}
...
void trace_mark_end(unsigned char* args) {
	if (trace_mark_enabled()) {
		trace_mark('E', current->pid, "", args, current->comm);
	}
}
...
void trace_mark_start(
	...
	if (trace_mark_enabled()) {
		...
		trace_mark('S', current->pid, name, args, current->comm);
	}
}
...
void trace_mark_finish(
	...
	if (trace_mark_enabled()) {
		...
		trace_mark('F', current->pid, name, args, current->comm);
	}
}
